document.addEventListener("DOMContentLoaded", async function() {
    try {
        const response = await fetch("D:\CMPS350\projects\cmps350group7\mizan_app\mizan-data\courses.json" )
        ...
        if (!newresponse.ok) {
            throw new Error(`Failed to fetch data: ${newresponse.status}`);
        }
        if (!typeresponse.ok) {
            throw new Error(`Failed to fetch data: ${typeresponse.status}`);
        }
        if (!deptresponse.ok) {
            throw new Error(`Failed to fetch data: ${deptresponse.status}`);
        }
